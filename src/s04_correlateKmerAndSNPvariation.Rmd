...
# create species binomial column
wcvp_names$species = paste(wcvp_names$genus, wcvp_names$species, sep = " ")

# drop names that are not in diversity dataset
wcvp_names = wcvp_names[(wcvp_names$species %in% alldiv$species),]

# keep only name id -> species key
wcvp_names = unique(wcvp_names[,c("plant_name_id", "species")])

# drop areas that don't match plant ids
wcvp_dist = wcvp_dist[(wcvp_dist$plant_name_id %in% wcvp_names$plant_name_id),]

# remove areas that are introduced, extinct, or dubious
wcvp_dist = wcvp_dist[(wcvp_dist$introduced == 0 & wcvp_dist$extinct == 0 & wcvp_dist$location_doubtful == 0),]

# add in species names to distributions
wcvp_dist_names = merge(wcvp_dist, wcvp_names, by = "plant_name_id")

# calculate area of countries for each species

code_to_area(x){
  # get country polygons from area codes
  
  # calculate areas of polygons
  
}
for(species in unique(wcvp_dist_names$species)){
  wcvp_dist_sub = 
