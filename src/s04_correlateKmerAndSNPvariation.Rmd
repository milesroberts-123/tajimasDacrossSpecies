...
coverage = read.table("../results/2023-03-26/bp_sequenced_2023-10-16.txt", header = T, sep = "\t")
...
phy = read.tree("../results/2023-03-26/species_list_2024-01-17.nwk")

# remove underscores
phy$tip.label = gsub("_", " ", phy$tip.label)

# remove variety and subspecies names
...
# check that all species I have data for are in phylogeny. Output should be TRUE
...
metadata = read.table("../results/2023-03-26/species_metadata_2024-01-24.txt", sep = "\t", fill = T, quote = "", header = T)
...
alldiv = merge(alldiv, metadata[,c("species", "DNA.amount..1.C..pg.", "Mating.system", "Ploidy", "Height..m.")], by = "species", all.x = T)
...
alldiv$GBIF.popsize = (alldiv$rangeArea * 1e6)/(alldiv$height_final^2)
...
my_ggplot_wrap <- function(data, xcol, ycol, xname, yname, filename, filew = 7, fileh = 7) {
  ggplot(data = data, aes_string(x = xcol, y = ycol)) + 
    geom_point() +
...
my_ggplot_wrap_xyz <- function(data, xcol, ycol, xname, yname, filename, zcol, lcol, hcol, lname, filew = 9, fileh = 7) {
  ggplot(data = data, aes_string(x = xcol, y = ycol, color = zcol)) + 
    geom_point() +
    geom_smooth(method = "loess", color = "pink") +
    theme_classic() +
    scale_color_gradient(low = lcol, high = hcol, name = lname) +
    labs(x = xname, y = yname)
  
  ggsave(filename, width = filew, height = fileh)
}

my_ggplot_wrap(alldiv, "log10(totalbp/(nidv*genome.size))", "log10(pi)", "log10(Average coverage per individual)", "log10(Nucleotide diversity)", paste("ida_coverage-vs-pi_", today, ".png"))

my_ggplot_wrap(alldiv, "log10(totalbp/(nidv*genome.size))", "jac", "log10(Average coverage per individual)", "Jaccard dissimilarity", paste("ida_coverage-vs-jac_", today, ".png"))

my_ggplot_wrap(alldiv, "log10(totalbp/(nidv*genome.size))", "bcd", "log10(Average coverage per individual)", "Bray-Curtis dissimilarity", paste("ida_coverage-vs-bcd_", today, ".png"))

my_ggplot_wrap(alldiv, "cvbp", "log10(pi)", "Coefficient of variation in base pairs sequenced across all samples", "log10(Nucleotide diversity)", paste("ida_cvbp-vs-pi_", today, ".png"))

my_ggplot_wrap(alldiv, "cvbp", "jac", "Coefficient of variation in base pairs sequenced across all samples", "Jaccard dissimilarity", paste("ida_cvbp-vs-jac_", today, ".png"))

my_ggplot_wrap(alldiv, "cvbp", "bcd", "Coefficient of variation in base pairs sequenced across all samples", "Bray-Curtis dissimilarity", paste("ida_cvbp-vs-bcd_", today, ".png"))

my_ggplot_wrap_xyz(alldiv, "log10(pi)", "jac", "log10(Nucleotide diversity)", "Jaccard dissimilarity", paste("ida_pi-vs-jac_", today, ".png"), "log10(totalbp/(nidv*genome.size))", "#FEF2F2", "#190C64", "log10(mean cov/sample)")

my_ggplot_wrap_xyz(alldiv, "log10(pi)", "bcd", "log10(Nucleotide diversity)", "Bray-Curtis dissimilarity", paste("ida_pi-vs-bcd_", today, ".png"), "log10(totalbp/(nidv*genome.size))", "#FEF2F2", "#190C64", "log10(mean cov/sample)")

my_ggplot_wrap_xyz(alldiv, "jac", "bcd", "Jaccard dissimilarity", "Bray-Curtis dissimilarity", paste("ida_jac-vs-bcd_", today, ".png"), "log10(totalbp/(nidv*genome.size))", "#FEF2F2", "#190C64", "log10(mean cov/sample)")

my_ggplot_wrap(alldiv, "log10(WCVP.popsize)", "log10(GBIF.popsize)", "log10(WCVP population size proxy)", "log10(GBIF population size proxy)", paste("ida_wcvp-vs-gbif_", today, ".png"))
...
alldiv = alldiv[(log10(alldiv$totalbp/(alldiv$nidv*alldiv$genome.size)) > 0),]
```

## replot trends
```{r}
myggplotwrap(alldiv, "log10(totalbp/(nidv*genome.size))", "log10(pi)", "log10(Average coverage per individual)", "log10(Nucleotide diversity)", paste("cda_coverage-vs-pi_", today, ".png"))

myggplotwrap(alldiv, "log10(totalbp/(nidv*genome.size))", "jac", "log10(Average coverage per individual)", "Jaccard dissimilarity", paste("cda_coverage-vs-jac_", today, ".png"))

myggplotwrap(alldiv, "log10(totalbp/(nidv*genome.size))", "bcd", "log10(Average coverage per individual)", "Bray-Curtis dissimilarity", paste("cda_coverage-vs-bcd_", today, ".png"))

myggplotwrap(alldiv, "cvbp", "log10(pi)", "Coefficient of variation in base pairs sequenced across all samples", "log10(Nucleotide diversity)", paste("cda_cvbp-vs-pi_", today, ".png"))

myggplotwrap(alldiv, "cvbp", "jac", "Coefficient of variation in base pairs sequenced across all samples", "Jaccard dissimilarity", paste("cda_cvbp-vs-jac_", today, ".png"))

myggplotwrap(alldiv, "cvbp", "bcd", "Coefficient of variation in base pairs sequenced across all samples", "Bray-Curtis dissimilarity", paste("cda_cvbp-vs-bcd_", today, ".png"))

myggplotwrap(alldiv, "log10(pi)", "jac", "log10(Nucleotide diversity)", "Jaccard dissimilarity", paste("cda_pi-vs-jac_", today, ".png"))

myggplotwrap(alldiv, "log10(pi)", "bcd", "log10(Nucleotide diversity)", "Bray-Curtis dissimilarity", paste("cda_pi-vs-bcd_", today, ".png"))

myggplotwrap(alldiv, "jac", "bcd", "Jaccard dissimilarity", "Bray-Curtis dissimilarity", paste("cda_jac-vs-bcd_", today, ".png"))

myggplotwrap(alldiv, "log10(WCVP.popsize)", "log10(GBIF.popsize)", "log10(WCVP population size proxy)", "log10(GBIF population size proxy)", paste("cda_wcvp-vs-gbif_", today, ".png"))
```

## Combine tree and data to get comparative object
```{r}
# create comparative data object for caper models
compdata = comparative.data(phy, alldiv[,c("species","pi", "jac", "bcd", "genome.size", "totalbp", "nidv", "WCVP.popsize")], names.col = "species", vcv = T)
```

## fit models
```{r}
# check for multicollinarity before fitting models
cor(alldiv[,c("pi", "jac", "bcd", "genome.size", "totalbp", "nidv", "WCVP.popsize", "GBIF.popsize")], method = "spearman", use = "pairwise.complete.obs")
...
p0 = ggtree(plotphy, size = 1.5) %<+% plotdf
p1 = p0 + geom_tippoint(aes(shape = Mating.system), size = 1.5) + 
  geom_facet(panel = "Ploidy", data = plotdf, geom = geom_segment, size = 3, mapping = aes(x = 0, y = y, yend = y, xend = plotdf$Ploidy), color = plotpal[2]) +
  geom_facet(panel = "Genome size", data = plotdf, geom = geom_segment, size = 3, mapping = aes(x = 0, y = y, yend = y, xend = plotdf$genome.size), color = plotpal[3]) +
  geom_facet(panel = "Individuals", data = plotdf, geom = geom_segment, size = 3, mapping = aes(x = 0, y = y, yend = y, xend = plotdf$nidv), color = plotpal[4]) +
  geom_facet(panel = "Mean coverage per individual", data = plotdf, geom = geom_segment, size = 3, mapping = aes(x = 0, y = y, yend = y, xend = plotdf$totalbp/(plotdf$genome.size*plotdf$nidv)), color = plotpal[5]) +
  theme_tree2() +
  theme(strip.background =element_rect(fill="white")) +
  theme(strip.text = element_text(colour = 'black'))
