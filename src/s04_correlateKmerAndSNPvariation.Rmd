...
```{r eval=FALSE, include=FALSE}
alldiv = read.table("../results/2023-03-26/final_covariates_2024-02-21.txt", sep = "\t", header = T)
```
...
phy = read.tree("../results/2023-03-26/species_list_2024-02-21.nwk")
...
ida_wcvp_vs_gbif_native = my_ggplot_wrap_line(alldiv, "log10(wcvp_native_area)", "log10(gbif_native_area)", "log10(WCVP native range size, km^2)", "log10(GBIF native range size, km^2)", paste("ida_wcvp-vs-gbif_", today, ".png", sep = ""))

ida_wcvp_vs_gbif_all = my_ggplot_wrap_line(alldiv, "log10(wcvp_all_area)", "log10(gbif_all_area)", "log10(WCVP native+invaded range size, km^2)", "log10(GBIF native+invaded range size, km^2)", paste("ida_wcvp-vs-gbif_", today, ".png", sep = ""))
...
mod_wcvp_native_area_pi = pgls(scale(log10(pi)) ~ log10(wcvp_navtive_area) + Mating.system + cultivation.status + Generation.time, compdata)

mod_wcvp_native_area_jac = pgls(scale(jac) ~ log10(wcvp_navtive_area) + Mating.system + cultivation.status + Generation.time, compdata)

mod_wcvp_native_area_bcd = pgls(scale(bcd) ~ log10(wcvp_navtive_area) + Mating.system + cultivation.status + Generation.time, compdata)
