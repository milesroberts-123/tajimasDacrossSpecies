...
# Final plots
```{r}
ggplot(alldiv, aes(x = log10(rangeArea), y = pi, label = species)) +
  geom_point() +
  geom_text(position = position_jitter(seed = 1)) +
  theme_classic() +
  labs(x = "log10(Range area in square km)", y = "Nucleotide diversity")

ggsave("rangeVsPi_2023-04-25.png", width = 7, height = 7, unit = "in", scale = 0.5)

ggplot(alldiv, aes(x = log10(rangeArea), y = scale(bcd), label = species)) +
  geom_point() +
  geom_text(position = position_jitter(seed = 1)) + 
  theme_classic() +
  xlim(5.5, 8.5) +
  labs(x = "log10(Range area in square km)", y = "Scaled Bray-Curtis dissimilarity")
ggsave("rangeVsBray-Curtis_2023-04-25.png", width = 7, height = 7, unit = "in", scale = 0.5)

ggplot(alldiv, aes(x = log10(rangeArea), y = scale(jac), label = species)) +
  geom_point() +
  geom_text(position = position_jitter(seed = 1)) + 
  theme_classic() +
  xlim(5.5, 8.5) +
  labs(x = "log10(Range area in square km)", y = "Scaled Bray-Curtis dissimilarity")
ggsave("rangeVsJaccard_2023-04-25.png", width = 7, height = 7, unit = "in", scale = 0.5)
```