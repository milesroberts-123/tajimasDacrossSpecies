...
# calculate average number of k-mers included in union, scaled by dissimilarity
unionMean = lapply(jac, function(x){mean(x[,4]*x[,2])})
...
```{r eval=FALSE, include=FALSE}
alldiv = read.table("../results/2023-03-26/final_covariates_2024-03-02.txt", sep = "\t", header = T)
```
...
phy = read.tree("../results/2023-03-26/species_list_2024-03-02.nwk")
...
ida_generation_vs_mating = my_ggplot_wrap_barplot(alldiv, "Generation.time", "Mating.system", "Life cycle habit", "Number of species", "Mating system", c("#65014B", "#0C4C00"), paste("ida_generation_vs_mating_", today, ".png", sep = ""))

ida_cultivation_vs_mating = my_ggplot_wrap_barplot(alldiv, "cultivation.status", "Mating.system", "Cultivation status", "Number of species", "Mating system", c("#65014B", "#0C4C00"), paste("ida_cultivation_vs_mating_", today, ".png", sep = ""))

ida_cultivation_vs_generation = my_ggplot_wrap_barplot(alldiv, "cultivation.status", "Generation.time", "Cultivation status", "Number of species", "Life cycle habit", c("#9EB0FF", "#FFACAC"), paste("ida_cultivation_vs_generation_", today, ".png", sep = ""))
...
ida_wcvp_vs_gbif_native = my_ggplot_wrap_line(alldiv, "log10(wcvp_native_area)", "log10(gbif_native_area)", expression(bold("log10(WCVP native range size)"~(km^2))), expression(bold("log10(GBIF native range size)"~(km^2))), paste("ida_wcvp-vs-gbif_native_", today, ".png", sep = ""))

ida_wcvp_vs_gbif_all = my_ggplot_wrap_line(alldiv, "log10(wcvp_all_area)", "log10(gbif_all_area)", expression(bold("log10(WCVP native+invaded range size)"~km^2)), expression(bold("log10(GBIF native+invaded range size)"~(km)^2)), paste("ida_wcvp-vs-gbif_all_", today, ".png", sep = ""))

# compare number of individuals sequenced to diversity measures
ida_nidv_vs_pi = my_ggplot_wrap(alldiv, "log10(nidv)", "log10(pi)", "log(Number of biosamples sequenced)", "log(Nucleotide diversity)", paste("ida_nidv-vs-pi_", today, ".png", sep = ""))

ida_nidv_vs_jac = my_ggplot_wrap(alldiv, "nidv", "jac", "Number of biosamples sequenced", "Jaccard dissimilarity", paste("ida_nidv-vs-jac_", today, ".png", sep = ""))

ida_nidv_vs_bcd = my_ggplot_wrap(alldiv, "nidv", "bcd", "Number of biosamples sequenced", "Bray-Curtis dissimilarity", paste("ida_nidv-vs-bcd_", today, ".png", sep = ""))
...
cda_pi_vs_wcvp = my_ggplot_wrap(subdiv, "log10(pi)", "log10(wcvp_native_popsize)", "log10(Nucleotide diversity)", "log10(WCVP pop size proxy)", paste("cda_pi-vs-wcvp_", today, ".png", sep = ""))

cda_pi_vs_gbif = my_ggplot_wrap(subdiv, "log10(pi)", "log10(gbif_native_popsize)", "log10(Nucleotide diversity)", "log10(GBIF pop size proxy)", paste("cda_pi-vs-gbif_", today, ".png", sep = ""))

cda_jac_vs_wcvp = my_ggplot_wrap(subdiv, "jac", "log10(wcvp_native_popsize)", "Jaccard dissimilarity", "log10(WCVP pop size proxy)", paste("cda_jac-vs-wcvp_", today, ".png", sep = ""))

cda_jac_vs_gbif = my_ggplot_wrap(subdiv, "jac", "log10(gbif_native_popsize)", "Jaccard dissimilarity", "log10(GBIF pop size proxy)", paste("cda_jac-vs-gbif_", today, ".png", sep = ""))

cda_bcd_vs_wcvp = my_ggplot_wrap(subdiv, "bcd", "log10(wcvp_native_popsize)", "Bray-Curtis dissimilarity", "log10(WCVP pop size proxy)", paste("cda_bcd-vs-wcvp_", today, ".png", sep = ""))

cda_bcd_vs_gbif = my_ggplot_wrap(subdiv, "bcd", "log10(gbif_native_popsize)", "Bray-Curtis dissimilarity", "log10(GBIF pop size proxy)", paste("cda_bcd-vs-gbif_", today, ".png", sep = ""))
```
...
```{r eval=FALSE, include=FALSE}
summary(subdiv$jac/(subdiv$pi*30))
...
subdiv$excess_color = ifelse(subdiv$jac/(subdiv$pi*30) > 1, "greater", "lesser")

ggplot(subdiv, aes(x = reorder(species,-log10(jac/(pi*30))), y = log10(jac/(pi*30)), color = excess_color, fill = excess_color)) +
  geom_bar(stat = "identity") +
  theme_classic() +
...
ggarrange(ida_wcvp_vs_gbif_all, ida_wcvp_vs_gbif_native, ida_jac_vs_bcd, ida_generation_vs_mating, nrow = 2, ncol = 2, labels = c("A", "B", "C", "D"), font.label = list(size = 20))

ggsave(paste("wcvp_vs_gbif_and_jac_vs_bcd_", today, ".pdf", sep = ""), width = 16.5, height = 16)

# Look at how coverage influences metrics
ggarrange(ida_meancov_vs_pi, ida_cvbp_vs_pi, ida_meancov_vs_jac, ida_cvbp_vs_jac, ida_meancov_vs_bcd, ida_cvbp_vc_bcd, ncol = 2, nrow = 3, labels = c("A", "B", "C", "D", "E", "F"), font.label = list(size = 20))

ggsave(paste("coverage_vs_diversity_", today, ".pdf", sep = ""), width = 16, height = 16, units = "in")
...
ggsave(paste("diversity_vs_popsize_", today, ".pdf", sep = ""), width = 12, height = 16, units = "in")
