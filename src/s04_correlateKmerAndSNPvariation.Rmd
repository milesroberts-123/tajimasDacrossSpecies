...
alldiv$WCVP.popsize = (alldiv$area * 1e6)/(alldiv$height^2)
summary(alldiv$WCVP.popsize)

alldiv$GBIF.popsize = (alldiv$rangeArea * 1e6)/(alldiv$height^2)
summary(alldiv$GBIF.popsize)
```
## genome size, ploidy, mating system data
```{r}
# load extra species metadata
metadata = read.table("../results/2023-03-26/species_metadata_2023-10-16.txt", sep = "\t", fill = T, quote = "", header = T)
names(metadata)[1] = "species"

alldiv = merge(alldiv, metadata[,c("species", "DNA.amount..1.C..pg.", "Mating.system", "Ploidy")], by = "species", all.x = T)
...
ggtree(plotphy, size = 1.5) + 
    geom_facet(panel = "log10(GBIF Range area)", data = plotdf, geom = geom_segment, size = 3, mapping = aes(x = 0, y = y, yend = y, xend = log10(rangeArea)), color = plotpal[1]) +
    geom_facet(panel = "log10(WCVP Range area)", data = plotdf, geom = geom_segment, size = 3, mapping = aes(x = 0, y = y, yend = y, xend = log10(area)), color = plotpal[2]) +
...
ggtree(plotphy, size = 1.5) + 
  #geom_facet(panel = "Mating system", geom = geom_segment, size = 3, data = plotdf, mapping = aes(x = 0, color = Mating.system, yend = y, y = y, xend = 1)) +
  geom_tiplab(aes(x = 0, label = Mating.system)) +
  geom_facet(panel = "Ploidy", data = plotdf, geom = geom_segment, size = 3, mapping = aes(x = 0, y = y, yend = y, xend = Ploidy), color = plotpal[2]) +
  geom_facet(panel = "Genome size", data = plotdf, geom = geom_segment, size = 3, mapping = aes(x = 0, y = y, yend = y, xend = genome.size), color = plotpal[3]) +
  geom_facet(panel = "Individuals", data = plotdf, geom = geom_segment, size = 3, mapping = aes(x = 0, y = y, yend = y, xend = nidv), color = plotpal[4]) +
  geom_facet(panel = "Coverage", data = plotdf, geom = geom_segment, size = 3, mapping = aes(x = 0, y = y, yend = y, xend = totalbp/(genome.size*nidv)), color = plotpal[5]) +
  theme_tree2() +
  theme(strip.background =element_rect(fill="white")) +
  theme(strip.text = element_text(colour = 'black'))
