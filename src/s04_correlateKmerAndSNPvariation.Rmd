...
metadata = read.table("../results/2023-03-26/species_metadata_2024-01-26.txt", sep = "\t", fill = T, quote = "", header = T)
...
my_ggplot_wrap(alldiv, "log10(area)", "log10(gbif.area)", "log10(WCVP range size in square km)", "log10(GBIF range size in square km)", paste("ida_wcvp-vs-gbif_", today, ".png", sep = ""))
...
my_ggplot_wrap(subdiv, "log10(WCVP.popsize)", "log10(GBIF.popsize)", "log10(WCVP population size proxy)", "log10(GBIF population size proxy)", paste("cda_wcvp-vs-gbif_", today, ".png"))
```

## Combine tree and data to get comparative object
```{r}
# create comparative data object for caper models
compdata = comparative.data(phy, subdiv[,c("species","pi", "jac", "bcd", "genome.size", "meancov", "nidv", "cvbp", "cultivation.status", "Mating.system", "WCVP.popsize")], names.col = "species", vcv = T)
```

## fit models
```{r}
# check for multicollinarity before fitting models
cor(subdiv[,c("pi", "jac", "bcd", "genome.size", "meancov", "nidv", "cvbp", "WCVP.popsize", "GBIF.popsize")], method = "spearman", use = "pairwise.complete.obs")

# build models 
mod_pi = pgls(log10(pi) ~ log10(WCVP.popsize) + log10(meancov) + nidv + log10(genome.size) + cvbp + cultivation.status + Mating.system, compdata)
summary(mod_pi)

mod_jac = pgls(log10(WCVP.popsize) ~ jac + log10(meancov) + nidv + log10(genome.size), compdata)
summary(mod_jac)

mod_bcd = pgls(log10(WCVP.popsize) ~ bcd + log10(meancov) + nidv + log10(genome.size), compdata)
summary(mod_bcd)
...
# Look at how mating system and cultivation status influence diversity
ggarrange(ida_mating_vs_pi, ida_cult_vs_pi, ida_mating_vs_jac, ida_cult_vs_jac, ida_mating_vs_bcd, ida_cult_vs_bcd, ncol = 2, nrow = 3, labels = c("A", "B", "C", "D", "E", "F"))

ggsave(paste("mating_cultivation_vs_diversity_", today, ".png", sep = ""), width = 8, height = 10, units = "in")

# Compare how k-mer metrics compare to SNP metrics after removing low coverage data
ggarrange(ida_pi_vs_jac, ida_pi_vs_bcd, cda_pi_vs_jac, cda_pi_vs_bcd, ncol = 2, nrow = 2, common.legend = T, labels = c("A", "B", "C", "D"))
...
my_ggplot_wrap(subdiv, "log10(pi)", "log10(WCVP.popsize)", "log10(Nucleotide diversity)", "log10(WCVP pop size proxy)", paste("cda_pi-vs-wcvp_", today, ".png"))

my_ggplot_wrap(subdiv, "log10(pi)", "log10(GBIF.popsize)", "log10(Nucleotide diversity)", "log10(GBIF pop size proxy)", paste("cda_pi-vs-gbif_", today, ".png"))

my_ggplot_wrap(subdiv, "jac", "log10(WCVP.popsize)", "Jaccard dissimilarity", "log10(WCVP pop size proxy)", paste("cda_jac-vs-wcvp_", today, ".png"))

my_ggplot_wrap(subdiv, "jac", "log10(GBIF.popsize)", "Jaccard dissimilarity", "log10(GBIF pop size proxy)", paste("cda_jac-vs-gbif_", today, ".png"))

my_ggplot_wrap(subdiv, "bcd", "log10(WCVP.popsize)", "Bray-Curtis dissimilarity", "log10(WCVP pop size proxy)", paste("cda_bcd-vs-wcvp_", today, ".png"))

my_ggplot_wrap(subdiv, "bcd", "log10(GBIF.popsize)", "Bray-Curtis dissimilarity", "log10(GBIF pop size proxy)", paste("cda_bcd-vs-gbif_", today, ".png"))
```
