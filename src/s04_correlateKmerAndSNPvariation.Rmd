...
phy = read.tree("../results/2023-03-26/species_list_2023-10-16.nwk")
...
```{r eval=FALSE, include=FALSE}
ggplot(alldiv, aes(x = log10(totalbp/nidv), y = log10(pi), label = species)) +
  geom_point() +
  geom_text(position = position_jitter(seed = 1)) +
  theme_classic() +
  labs(x = "log10(Average number of base pairs sequenced per individual)", y = "log10(Nucleotide diversity)")
...
ggplot(alldiv, aes(x = log10(pi), y = bcd, label = species)) +
  geom_point() +
  geom_text(position = position_jitter(seed = 1)) +
  theme_classic() +
  labs(x = "log10(Nucleotide diversity)", y = "Bray-Curtis Dissimilarity")

ggplot(alldiv, aes(x = log10(pi), y = jac, label = species)) +
  geom_point() +
  geom_text(position = position_jitter(seed = 1)) +
  theme_classic() +
...
ggtree(plotphy, size = 1.5) + 
    geom_facet(panel = "GBIF range (km^2)", data = plotdf, geom = geom_segment, size = 3, mapping = aes(x = 0, y = y, yend = y, xend = rangeArea), color = plotpal[1]) +
    geom_facet(panel = "WCVP range (km^2)", data = plotdf, geom = geom_segment, size = 3, mapping = aes(x = 0, y = y, yend = y, xend = area), color = plotpal[2]) +
...
ggsave(paste("../results/2023-03-26/tree_with_primary_variables_", today, ".png", sep = ""), scale = 2)
...
ggsave(paste("../results/2023-03-26/tree_with_technical_variables_", today, ".png", sep = ""), scale = 2)
