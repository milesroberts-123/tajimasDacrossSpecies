...
# Prunus avidum is native to europe, asia, africa
...
my_ggplot_wrap_line(alldiv, "log10(area)", "log10(gbif.area)", "log10(WCVP range size in square km)", "log10(GBIF range size in square km)", paste("ida_wcvp-vs-gbif_", today, ".png", sep = ""))
...
correct_and_plot = function(mod_mat, y, V, primary, xlab, ylab){
  ...
  myplot = ggplot(mapping = aes(x = X_star[,primary], y = y_star_corrected)) +
    ...
    annotate(geom = "text", x = 3, y = (max(y_star_corrected) + min(y_star_corrected))*0.5, label = p.label) +
    labs(x = xlab, y = ylab)
    
  return(myplot)
}

wcvp_vs_pi_corrected = correct_and_plot(mod_mat_pi, mod_pi_1$y, mod_pi_1$Vt, "log10(WCVP.popsize)", "log10(WCVP pop size proxy), scaled", "Nucleotide diversity, scaled and adjusted")

wcvp_vs_jac_corrected = correct_and_plot(mod_mat_jac, mod_jac_1$y, mod_jac_1$Vt, "log10(WCVP.popsize)", "log10(WCVP pop size proxy), scaled", "Jaccard dissimilarity, scaled and adjusted")

wcvp_vs_bcd_corrected = correct_and_plot(mod_mat_bcd, mod_bcd_1$y, mod_bcd_1$Vt, "log10(WCVP.popsize)", "log10(WCVP pop size proxy), scaled", "Bray-Curtis dissimilarity, scaled and adjusted")
...
ggarrange(wcvp_vs_pi_corrected, wcvp_vs_jac_corrected, wcvp_vs_bcd_corrected, nrow = 1, ncol = 3, labels = c("A", "B", "C"))

ggsave(paste("wcpv_vs_diversity_corrected_", today, ".png", sep = ""), height = 4.5, width = 13.5, units = "in")
```
