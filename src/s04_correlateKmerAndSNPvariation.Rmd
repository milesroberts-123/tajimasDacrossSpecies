...
metadata = read.table("../results/2023-03-26/species_metadata_2024-01-19.txt", sep = "\t", fill = T, quote = "", header = T)
...
```{r}
ggplot(alldiv, aes(x = log10(totalbp/(nidv*genome.size)), y = log10(pi))) +
  geom_point() +
  #geom_text(position = position_jitter(seed = 1)) +
  theme_classic() +
  labs(x = "log10(Average coverage per individual)", y = "log10(Nucleotide diversity)")

ggsave(paste("ida_coverage-vs-pi_", today, ".png"))

ggplot(alldiv, aes(x = log10(totalbp/(nidv*genome.size)), y = jac)) +
  geom_point() +
  #geom_text(position = position_jitter(seed = 1)) +
  theme_classic() +
  labs(x = "log10(Average coverage per individual)", y = "Jaccard dissimilarity")

ggsave(paste("ida_coverage-vs-jac_", today, ".png"))

ggplot(alldiv, aes(x = log10(totalbp/(nidv*genome.size)), y = bcd)) +
  geom_point() +
  #geom_text(position = position_jitter(seed = 1)) +
  theme_classic() +
  labs(x = "log10(Average coverage per individual)", y = "Bray-Curtis dissimilarity")

ggsave(paste("ida_coverage-vs-bcd_", today, ".png"))

ggplot(alldiv, aes(x = cvbp, y = log10(pi))) +
  geom_point() +
  #geom_text(position = position_jitter(seed = 1)) +
  theme_classic() +
  labs(x = "Coefficient of variation in base pairs sequenced across all samples", y = "log10(Nucleotide diversity)")

ggsave(paste("ida_cvbp-vs-pi_", today, ".png"))

ggplot(alldiv, aes(x = cvbp, y = jac)) +
  geom_point() +
  #geom_text(position = position_jitter(seed = 1)) +
  theme_classic() +
  labs(x = "Coefficient of variation in base pairs sequenced across all samples", y = "Jaccard dissimilarity")

ggsave(paste("ida_cvbp-vs-jac_", today, ".png"))

ggplot(alldiv, aes(x = cvbp, y = bcd)) +
  geom_point() +
  #geom_text(position = position_jitter(seed = 1)) +
  theme_classic() +
  labs(x = "Coefficient of variation in base pairs sequenced across all samples", y = "Bray-Curtis dissimilarity")

ggsave(paste("ida_cvbp-vs-bcd_", today, ".png"))

ggplot(alldiv, aes(x = log10(pi), y = bcd)) +
  geom_point() +
  #geom_text(position = position_jitter(seed = 1)) +
  theme_classic() +
  labs(x = "log10(Nucleotide diversity)", y = "Bray-Curtis dissimilarity")

ggsave(paste("ida_pi_vs_bcd_", today, ".png", sep = ""))

ggplot(alldiv, aes(x = log10(pi), y = jac)) +
  geom_point() +
  #geom_text(position = position_jitter(seed = 1)) +
  theme_classic() +
  labs(x = "log10(Nucleotide diversity)", y = "Jaccard dissimilarity")

ggsave(paste("ida_pi_vs_jac_", today, ".png", sep = ""))

ggplot(alldiv, aes(x = bcd, y = jac)) +
  geom_point() +
  #geom_text(position = position_jitter(seed = 1), color = "deepskyblue") +
  theme_classic() +
  labs(x = "Bray-Curtis dissimilarity", y = "Jaccard dissimilarity")

ggsave(paste("ida_bcd_vs_jac_", today, ".png", sep = ""))

# compare the different population size metrics

ggplot(alldiv, aes(x = log10(WCVP.popsize), y = log10(GBIF.popsize))) +
  geom_point() +
  #geom_text(position = position_jitter(seed = 1), color = "deepskyblue") +
  theme_classic() +
  labs(x = "log10(WCVP population size proxy)", y = "log10(GBIF population size proxy)")

ggsave(paste("ida_wcvp-vs-gbif_", today, ".png", sep = ""))
...
p1 = p0 + geom_tippoint(aes(shape = Mating.system), size = 1.5) + 
  ...
  geom_facet(panel = "Coverage", data = plotdf, geom = geom_segment, size = 3, mapping = aes(x = 0, y = y, yend = y, xend = plotdf$totalbp/(plotdf$genome.size*plotdf$nidv)), color = plotpal[5]) +
  theme_tree2() +
  theme(strip.background =element_rect(fill="white")) +
  theme(strip.text = element_text(colour = 'black'))
