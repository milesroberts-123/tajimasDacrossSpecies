...
# add reads to growing list
peReads = rbind(peReads, data.frame(samples = glycine_max[(glycine_max$spots_with_mates > 0),"Run"], genome = "glycine_max"))
...
# add reads to growing list
peReads = rbind(peReads, data.frame(samples = panicum_halli[(panicum_halli$spots_with_mates > 0),"Run"], genome = "panicum_halli"))
...
# add reads to growing list
peReads = rbind(peReads, data.frame(samples = zea_mays[(zea_mays$spots_with_mates > 0),"Run"], genome = "zea_mays"))
...
write.table(peReads, file = "samples_pe.tsv", sep = "\t", quote = F, row.names = F)
```
