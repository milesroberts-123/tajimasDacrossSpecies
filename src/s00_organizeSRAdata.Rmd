...
# set random number generation seed using randomly picked seed
#sample(1:(.Machine$integer.max), size = 1)
set.seed(1577935165)

# load libraries
library(data.table)

# maximum number of base pairs to process for each species
# This is based on the number of basepairs in the brassica napus dataset, which I was just barely able to process on the MSU HPCC
maxbp = 7.5e12
...
## Functions
```{r}
# take SRA dataframe and maximum number of basepairs
# output random sample where total number of bp does not exceed maximum
random_subset = function(x, maxbp){
  output = NULL
  print("Begin random sampling...")
  while(sum(output$bases) < maxbp){
    percent_complete = round(sum(output$bases)/maxbp * 100, digits = 1)
    if(percent_complete %% 1 == 0){
      print(paste("Percent complete:", percent_complete))
    }
    row_to_add = sample(1:nrow(x), replace = F, size = 1) # randomly sample
    output = rbind(output, x[row_to_add,]) # save randomly sampled row 
    x = x[-row_to_add,] # drop row so that it's not sampled again
  }
  return(output)
  print("Done! :)")
}
...
allReads = rbind(allReads, data.frame(run = actinidia_chinensis[,"Run"], replicate = actinidia_chinensis[,"replicate"], layout = "SE", genome = "actinidia_chinensis", ploidy = 2, taxonKeys = "3189692 7379759 7620678 7879124"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = arabis_alpina[,"Run"], replicate = arabis_alpina[,"BioSample"], layout = arabis_alpina[,"layout"], genome = "arabis_alpina", ploidy = 2, taxonKeys = "5375020 5374885 8576900 9044273 7694599 8349130 5375026 5551157 7785553 9614998"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = arabis_nemorensis[,"Run"], replicate = arabis_nemorensis[,"BioSample"], layout = "SE", genome = "arabis_nemorensis", ploidy = 2, taxonKeys = "8288982 5374937 5552336 7292834 5374934"))
```

## Arachis hypogaea and Arachis duranensis and Arachis ipaensis
```{r}
# load GBS data for A. hypogaea
# arachis_hypogaea = fread("../workflow/data/SRArunInfo/SraRunInfo_arachis_hypogaea_PRJNA479538.csv.gz", data.table = F)
...
# remove large sample that will hold up entire pipeline
arachis_duranensis = arachis_duranensis[(arachis_duranensis$Run != "SRR8784099"),]

# add reads to growing list
# arachis hypogae is a tetraploid, but both subgenomes are in assembly, so we can treat it as a diploid for genotype calling
allReads = rbind(allReads, data.frame(run = arachis_hypogaea[,"Run"], replicate = arachis_hypogaea[,"BioSample"], layout = "PE", genome = "arachis_hypogaea", ploidy = 2, taxonKeys = "5353813"))

allReads = rbind(allReads, data.frame(run = arachis_duranensis[,"Run"], replicate = arachis_duranensis[,"BioSample"], layout = "PE", genome = "arachis_duranensis", ploidy = 2, taxonKeys = "5353841"))

allReads = rbind(allReads, data.frame(run = arachis_ipaensis[,"Run"], replicate = arachis_ipaensis[,"BioSample"], layout = "PE", genome = "arachis_ipaensis", ploidy = 2, taxonKeys = "5353758"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = benincasa_hispida[,"Run"], replicate = benincasa_hispida[,"BioSample"], layout = "PE", genome = "benincasa_hispida", ploidy = 2, taxonKeys = "3623456 7317644 2874524 3623445 3623491 8312084 2874525 7317643"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = beta_vulgaris[,"Run"], replicate = beta_vulgaris[,"BioSample"], layout = beta_vulgaris[,"layout"], genome = "beta_vulgaris", ploidy = 2, taxonKeys = "8372932 7657202"))
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = boechera_stricta[,"Run"], replicate = boechera_stricta[,"LibraryName"], layout = boechera_stricta[,"layout"], genome = "boechera_stricta", ploidy = 2, taxonKeys = "3043392"))
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = brachypodium_distachyon[,"Run"], replicate = brachypodium_distachyon[,"BioSample"], layout = brachypodium_distachyon[,"layout"], genome = "brachypodium_distachyon", ploidy = 2, taxonKeys = "5290143"))
```

## Brachypodium sylvaticum, under fort lauderdale accord
```{r eval=FALSE, include=FALSE}
# load data for species
brachypodium_sylvaticum = fread("../workflow/data/SRArunInfo/SraRunInfo_brachypodium_sylvaticum_PRJNA476867.csv.gz", data.table = F)

# add reads to growing list
allReads = rbind(allReads, data.frame(run = brachypodium_sylvaticum[,"Run"], replicate = brachypodium_sylvaticum[,"BioSample"], layout = "SE", genome = "brachypodium_sylvaticum", ploidy = 2, taxonKeys = "5290146"))
```
...
# add reads to growing list
# Brassica juncea is an allotetraploid, but it has two subgenomes with two copies of each subgenome. Therefore, we'll treat it like two diploid genomes
allReads = rbind(allReads, data.frame(run = brassica_juncea[,"Run"], replicate = brassica_juncea[,"BioSample"], layout = brassica_juncea[,"layout"], genome = "brassica_juncea", ploidy = 2, taxonKeys = "3042624"))
```
...
# ploidy is not actually 2, but we can treat it like a diploid for genotype calling because it's allopolyploid
allReads = rbind(allReads, data.frame(run = brassica_napus[,"Run"], replicate = brassica_napus[,"Experiment"], layout = "PE", genome = "brassica_napus", ploidy = 2, taxonKeys = "3042551"))
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = brassica_nigra[,"Run"], replicate = brassica_nigra[,"BioSample"], layout = "PE", genome = "brassica_nigra", ploidy = 2, taxonKeys = "3042658 8010384 7509066 3042673 3692032 3042665 7437326 6306670 7844947"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = brassica_rapa[,"Run"], replicate = brassica_rapa[,"BioSample"], layout = brassica_rapa[,"layout"], genome = "brassica_rapa", ploidy = 2, taxonKeys = "3042809"))

allReads = rbind(allReads, data.frame(run = brassica_oleracea[,"Run"], replicate = brassica_oleracea[,"BioSample"], layout = brassica_oleracea[,"layout"], genome = "brassica_oleracea", ploidy = 2, taxonKeys = "3042837"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = buddleja_alternifolia[,"Run"], replicate = buddleja_alternifolia[,"BioSample"], layout = "PE", genome = "buddleja_alternifolia", ploidy = 2, taxonKeys = "3173337"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = cajanus_cajan[,"Run"], replicate = cajanus_cajan[,"BioSample"], layout = "PE", genome = "cajanus_cajan", ploidy = 2, taxonKeys = "2960284"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = camelina_sativa[,"Run"], replicate = camelina_sativa[,"BioSample"], layout = "PE", genome = "camelina_sativa", ploidy = 6, taxonKeys = "3042439 7528991 3042443 3042441 3689020 3042425 3042427 3042396"))
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = camellia_sinensis[,"Run"], replicate = camellia_sinensis[,"BioSample"], layout = camellia_sinensis[,"layout"], genome = "camellia_sinensis", ploidy = 2, taxonKeys = "8105068"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = cannabis_sativa[,"Run"], replicate = cannabis_sativa[,"BioSample"], layout = "PE", genome = "cannabis_sativa", ploidy = 2, taxonKeys = "5361880"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = capsella_grandiflora[,"Run"], replicate = capsella_grandiflora[,"BioSample"], layout = capsella_grandiflora[,"layout"], genome = "capsella_grandiflora", ploidy = 2, taxonKeys = "5375387"))
```
...
# this one sample is single end
capsella_rubella$layout[(capsella_rubella$Run == "ERR636156")] = "SE"

# add reads to growing list
allReads = rbind(allReads, data.frame(run = capsella_rubella[,"Run"], replicate = capsella_rubella[,"BioSample"], layout = capsella_rubella[,"layout"], genome = "capsella_rubella", ploidy = 2, taxonKeys = "5375425"))
```
...
capsicum_annuum = capsicum_annuum[(capsicum_annuum$BioSample %in% sample(capsicum_annuum$BioSample, size = 1200, replace = F)),]

# add reads to growing list
allReads = rbind(allReads, data.frame(run = capsicum_annuum[,"Run"], replicate = capsicum_annuum[,"BioSample"], layout = "SE", genome = "capsicum_annuum", ploidy = 2, taxonKeys = "3799221"))
...
# remove oxford nanopore reads
castanea_mollissima = castanea_mollissima[!(castanea_mollissima$Platform == "OXFORD_NANOPORE"),]

# subset to just one species
castanea_mollissima = castanea_mollissima[(castanea_mollissima$ScientificName == "Castanea mollissima"),]

# add reads to growing list
allReads = rbind(allReads, data.frame(run = castanea_mollissima[,"Run"], replicate = castanea_mollissima[,"BioSample"], layout = "SE", genome = "castanea_mollissima", ploidy = 2, taxonKeys = "5333237"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = chenopodium_quinoa[,"Run"], replicate = chenopodium_quinoa[,"BioSample"], layout = chenopodium_quinoa[,"layout"], genome = "chenopodium_quinoa", ploidy = 4, taxonKeys = "3083905"))
```
...
# randomly sample 
cicer_arietinum = random_subset(cicer_arietinum, maxbp = maxbp)

# add reads to growing list
allReads = rbind(allReads, data.frame(run = cicer_arietinum[,"Run"], replicate = cicer_arietinum[,"BioSample"], layout = cicer_arietinum[,"layout"], genome = "cicer_arietinum", ploidy = 2, taxonKeys = "2947330"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = citrullus_lanatus[,"Run"], replicate = citrullus_lanatus[,"BioSample"], layout = citrullus_lanatus[,"layout"], genome = "citrullus_lanatus", ploidy = 2, taxonKeys = "3625134"))
```
...
allReads = rbind(allReads, data.frame(run = coffea_arabica[,"Run"], replicate = coffea_arabica[,"Experiment"], layout = "PE", genome = "coffea_arabica", ploidy = 4, taxonKeys = "7994689"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = coffea_canephora[,"Run"], replicate = coffea_canephora[,"BioSample"], layout = "PE", genome = "coffea_canephora", ploidy = 2, taxonKeys = "2895317"))
...
```{r}
# load data
corylus_americana = fread("../workflow/data/SRArunInfo/SraRunInfo_corylus_americana_PRJNA625120.csv.gz", data.table = F)

# check for dups
any(duplicated(corylus_americana$Run))
any(duplicated(corylus_americana$BioSample))
any(duplicated(corylus_americana$SampleName))

# add reads to growing list
allReads = rbind(allReads, data.frame(run = corylus_americana[,"Run"], replicate = corylus_americana[,"BioSample"], layout = "SE", genome = "corylus_americana", ploidy = 2, taxonKeys = "2895317"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = corylus_avellana[,"Run"], replicate = corylus_avellana[,"BioSample"], layout = "PE", genome = "corylus_avellana", ploidy = 2, taxonKeys = "2876032"))
```
...
# remove reads from outgroup based on source publication, 
# see Supplementary table 1 of source publication
runs2drop = paste("MS-", c(893:898, 900, 901, 995), sep = "")

cucumis_melo = cucumis_melo[!(cucumis_melo$SampleName %in% runs2drop),]

# add reads to growing list
allReads = rbind(allReads, data.frame(run = cucumis_melo[,"Run"], replicate = cucumis_melo[,"BioSample"], layout = "PE", genome = "cucumis_melo", ploidy = 2, taxonKeys = "7885624"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = cucumis_sativus[,"Run"], replicate = cucumis_sativus[,"BioSample"], layout = cucumis_sativus[,"layout"], genome = "cucumis_sativus", ploidy = 2, taxonKeys = "3625708"))
```
...
allReads = rbind(allReads, data.frame(run = cucurbita_argyrosperma[,"Run"], replicate = cucurbita_argyrosperma[,"BioSample"], layout = "SE", genome = "cucurbita_argyrosperma", ploidy = 2, taxonKeys = "3623807"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = cucurbita_pepo[,"Run"], replicate = cucurbita_pepo[,"BioSample"], layout = "SE", genome = "cucurbita_pepo", ploidy = 2, taxonKeys = "2874513"))
```
...
# add reads to growing list
# this species is tetraploid, but because two subgenomes are given in the genome assembly, we'll treat it as diploid
# each of the subgenomes has two copies, making for 4 copies in total (2x2=4)
allReads = rbind(allReads, data.frame(run = digitaria_exilis[,"Run"], replicate = digitaria_exilis[,"BioSample"], layout = "PE", genome = "digitaria_exilis", ploidy = 2, taxonKeys = "5289974"))
```
...
# only use accessions with a known ploidy of 2
dioscorea_alata = dioscorea_alata[(dioscorea_alata$`Inferred Ploidy` == 2) & !is.na(dioscorea_alata$`Inferred Ploidy`),]
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = dioscorea_alata[,"Run"], replicate = dioscorea_alata[,"BioSample"], layout = "SE", genome = "dioscorea_alata", ploidy = dioscorea_alata$`Inferred Ploidy`, taxonKeys = "2754392 2754837"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = dioscorea_rotundata[,"Run"], replicate = dioscorea_rotundata[,"BioSample"], layout = "PE", genome = "dioscorea_rotundata", ploidy = 4, taxonKeys = "2755063"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = durio_zibethinus[,"Run"], replicate = durio_zibethinus[,"BioSample"], layout = "PE", genome = "durio_zibethinus", ploidy = 2, taxonKeys = "4073136"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = elaeis_guineensis[,"Run"], replicate = elaeis_guineensis[,"BioSample"], layout = "PE", genome = "elaeis_guineensis", ploidy = 2, taxonKeys = "8791596"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = eleusine_coracana[,"Run"], replicate = eleusine_coracana[,"BioSample"], layout = "PE", genome = "eleusine_coracana", ploidy = 4, taxonKeys = "2705953"))
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = ficus_carica[,"Run"], replicate = ficus_carica[,"BioSample"], layout = "PE", genome = "ficus_carica", ploidy = 2, taxonKeys = "5361926"))
...
# random subset
glycine_max = random_subset(glycine_max, maxbp = maxbp)

# add reads to growing list
allReads = rbind(allReads, data.frame(run = glycine_max[,"Run"], replicate = glycine_max[,"BioSample"], layout = glycine_max[,"layout"], genome = "glycine_max", ploidy = 2, taxonKeys = "5359681"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = glycine_soja[,"Run"], replicate = glycine_soja[,"BioSample"], layout = glycine_soja[,"layout"], genome = "glycine_soja", ploidy = 2, taxonKeys = "5359668"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = gossypium_arboreum[,"Run"], replicate = gossypium_arboreum[,"BioSample"], layout = "PE", genome = "gossypium_arboreum", ploidy = 2, taxonKeys = "3152663"))
```
...
# add reads to growing list
# real ploidy is 4, but there are assemblies for both subgenomes, allotetraploid
allReads = rbind(allReads, data.frame(run = gossypium_barbadense[,"Run"], replicate = gossypium_barbadense[,"BioSample"], layout = gossypium_barbadense[,"layout"], genome = "gossypium_barbadense", ploidy = 2, taxonKeys = "3152666"))
```
...
# randomly subset
gossypium_hirsutum = random_subset(gossypium_hirsutum, maxbp = maxbp)

# add reads to growing list
allReads = rbind(allReads, data.frame(run = gossypium_hirsutum[,"Run"], replicate = gossypium_hirsutum[,"BioSample"], layout = gossypium_hirsutum[,"layout"], genome = "gossypium_hirsutum", ploidy = 2, taxonKeys = "3152653"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = helianthus_annuus[,"Run"], replicate = helianthus_annuus[,"BioSample"], layout = helianthus_annuus[,"layout"], genome = "helianthus_annuus", ploidy = 2, taxonKeys = "3119206"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = heliosperma_pusillum[,"Run"], replicate = heliosperma_pusillum[,"BioSample"], layout = "SE", genome = "heliosperma_pusillum", ploidy = 2, taxonKeys = "7267415"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = hordeum_vulgare[,"Run"], replicate = hordeum_vulgare[,"BioSample"], layout = hordeum_vulgare[,"layout"], genome = "hordeum_vulgare", ploidy = 2, taxonKeys = "6313800 8151496"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = juglans_regia[,"Run"], replicate = juglans_regia[,"BioSample"], layout = "PE", genome = "juglans_regia", ploidy = 2, taxonKeys = "4205640"))
```
...
lactuca_sativa$layout = "PE"
#lactuca_sativa$layout[(lactuca_sativa$spots_with_mates == 0)] = "SE"

# add reads to growing list
allReads = rbind(allReads, data.frame(run = lactuca_sativa[,"Run"], replicate = lactuca_sativa[,"BioSample"], layout = lactuca_sativa[,"layout"], genome = "lactuca_sativa", ploidy = 2, taxonKeys = "3140467"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = lens_culinaris[,"Run"], replicate = lens_culinaris[,"BioSample"], layout = "SE", genome = "lens_culinaris", ploidy = 2, taxonKeys = "3972444"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = lens_ervoides[,"Run"], replicate = lens_ervoides[,"BioSample"], layout = "SE", genome = "lens_ervoides", ploidy = 2, taxonKeys = "5350011"))
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = linum_usitatissimum[,"Run"], replicate = linum_usitatissimum[,"BioSample"], layout = linum_usitatissimum[,"layout"], genome = "linum_usitatissimum", ploidy = 2, taxonKeys = "2873896"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = lotus_japonicus[,"Run"], replicate = lotus_japonicus[,"BioSample"], layout = lotus_japonicus[,"layout"], genome = "lotus_japonicus", ploidy = 2, taxonKeys = "5357027"))
```
...
allReads = rbind(allReads, data.frame(run = lupinus_angustifolius[,"Run"], replicate = lupinus_angustifolius[,"Experiment"], layout = "PE", genome = "lupinus_angustifolius", ploidy = 2, taxonKeys = "2964374"))
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = macadamia_integrifolia[,"Run"], replicate = macadamia_integrifolia[,"BioSample"], layout = "SE", genome = "macadamia_integrifolia", ploidy = 2, taxonKeys = "8406703"))
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = malus_sylvestris[,"Run"], replicate = malus_sylvestris[,"BioSample"], layout = "PE", genome = "malus_sylvestris", ploidy = 2, taxonKeys = "3001509"))

allReads = rbind(allReads, data.frame(run = malus_domestica[,"Run"], replicate = malus_domestica[,"BioSample"], layout = "PE", genome = "malus_domestica", ploidy = 2, taxonKeys = "7864241"))
```
...
allReads = rbind(allReads, data.frame(run = mangifera_indica[,"Run"], replicate = mangifera_indica[,"BioSample"], layout = "PE", genome = "mangifera_indica", ploidy = 2, taxonKeys = "3190640"))
```
...
# some reads without mates are actually PE
manihot_esculenta$layout[(manihot_esculenta$Run %in% c("SRR1261957", "SRR1250413", "SRR1261704", "SRR1261701", "SRR1261610"))] = "PE"

# add reads to growing list
allReads = rbind(allReads, data.frame(run = manihot_esculenta[,"Run"], replicate = manihot_esculenta[,"BioSample"], layout = manihot_esculenta[,"layout"], genome = "manihot_esculenta", ploidy = 2, taxonKeys = "3060733"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = medicago_truncatula[,"Run"], replicate = medicago_truncatula[,"BioSample"], layout = medicago_truncatula[,"layout"], genome = "medicago_truncatula", ploidy = 2, taxonKeys = "2965201"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = mimulus_guttatus[,"Run"], replicate = mimulus_guttatus[,"BioSample"], layout = mimulus_guttatus[,"layout"], genome = "mimulus_guttatus", ploidy = 2, taxonKeys = "7346090"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = miscanthus_sinensis[,"Run"], replicate = miscanthus_sinensis[,"BioSample"], layout = "SE", genome = "miscanthus_sinensis", ploidy = 2, taxonKeys = "7738766"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = momordica_charantia[,"Run"], replicate = momordica_charantia[,"BioSample"], layout = "PE", genome = "momordica_charantia", ploidy = 2, taxonKeys = "3625538"))
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = musa_acuminata[,"Run"], replicate = musa_acuminata[,"BioSample"], layout = "PE", genome = "musa_acuminata", ploidy = 3, taxonKeys = "2762950"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = nelumbo_nucifera[,"Run"], replicate = nelumbo_nucifera[,"BioSample"], layout = "PE", genome = "nelumbo_nucifera", ploidy = 2, taxonKeys = "7503234"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = nicotiana_tabacum[,"Run"], replicate = nicotiana_tabacum[,"BioSample"], layout = "PE", genome = "nicotiana_tabacum", ploidy = 4, taxonKeys = "2928783"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = olea_europaea[,"Run"], replicate = olea_europaea[,"BioSample"], layout = olea_europaea[,"layout"], genome = "olea_europaea", ploidy = 2, taxonKeys = "7506783"))
```

## DEPRECATED Oryza barthii
```{r eval=FALSE, include=FALSE}
...
allReads = rbind(allReads, data.frame(run = oryza_brachyantha[,"Run"], replicate = oryza_brachyantha[,"BioSample"], layout = "PE", genome = "oryza_brachyantha", ploidy = 2, taxonKeys = "4110272"))
```
...
allReads = rbind(allReads, data.frame(run = oryza_barthii[,"Run"], replicate = oryza_barthii[,"BioSample"], layout = "PE", genome = "oryza_barthii", ploidy = 2, taxonKeys = "2703457"))

allReads = rbind(allReads, data.frame(run = oryza_glaberrima[,"Run"], replicate = oryza_glaberrima[,"BioSample"], layout = "PE", genome = "oryza_glaberrima", ploidy = 2, taxonKeys = "2703464"))
```

## DEPRECATED Oryza glumipatula - same as O. rufipogon
```{r eval=FALSE, include=FALSE}
oryza_glumipatula = fread("../workflow/data/SRArunInfo/SraRunInfo_oryza_glumipatula_PRJDB4703.csv.gz", data.table = F)

allReads = rbind(allReads, data.frame(run = oryza_glumipatula[,"Run"], replicate = oryza_glumipatula[,"BioSample"], layout = "PE", genome = "oryza_glumipatula", ploidy = 2, taxonKeys = "4940610 11365001 5942013"))
```
...
allReads = rbind(allReads, data.frame(run = oryza_longistaminata[,"Run"], replicate = oryza_longistaminata[,"BioSample"], layout = "PE", genome = "oryza_longistaminata", ploidy = 2, taxonKeys = "2703462"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = oryza_meridionalis[,"Run"], replicate = oryza_meridionalis[,"BioSample"], layout = "PE", genome = "oryza_meridionalis", ploidy = 2, taxonKeys = "9167736"))
...
allReads = rbind(allReads, data.frame(run = oryza_punctata[,"Run"], replicate = oryza_punctata[,"BioSample"], layout = "PE", genome = "oryza_punctata", ploidy = 2, taxonKeys = "2703461 8663541 11356139 5941998"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = oryza_rufipogon[,"Run"], replicate = oryza_rufipogon[,"BioSample"], layout = "PE", genome = "oryza_rufipogon", ploidy = 2, taxonKeys = "2703463"))
```
...
# randomly subset by basepairs
#oryza_sativa = random_subset(oryza_sativa, maxbp = maxbp)

# randomly subset by number of individuals
oryza_sativa = oryza_sativa[(oryza_sativa$BioSample %in% sample(unique(oryza_sativa$BioSample), size = 1200, replace = F)),]

# add reads to growing list
allReads = rbind(allReads, data.frame(run = oryza_sativa[,"Run"], replicate = oryza_sativa[,"BioSample"], layout = oryza_sativa[,"layout"], genome = "oryza_sativa", ploidy = 2, taxonKeys = "4135983"))
```
...
panicum_hallii = fread("../workflow/data/SRArunInfo/SraRunInfo_panicum_halli.csv.gz", data.table = F)

# check for technical replicates
any(duplicated(panicum_hallii$Experiment))
any(duplicated(panicum_hallii$SampleName))
any(duplicated(panicum_hallii$BioSample))
...
# remove pacbio reads
panicum_hallii = panicum_hallii[(panicum_hallii$Platform != "PACBIO_SMRT"),]

# add layout
# reads without mates are actually PE
panicum_hallii$layout = NA
panicum_hallii$layout = "PE"
#panicum_halli$layout[(panicum_halli$spots_with_mates > 0)] = "PE"
#panicum_halli$layout[(panicum_halli$spots_with_mates == 0)] = "SE"

# modify layout for reads that seem PE but are actually SE
panicum_hallii$layout[(panicum_hallii$Run == "SRR097411")] = "SE"

# drop sample that has too much data to process within 7 day limit
panicum_hallii = panicum_hallii[(panicum_hallii$BioSample != "SAMN05660343"),]

# add reads to growing list
allReads = rbind(allReads, data.frame(run = panicum_hallii[,"Run"], replicate = panicum_hallii[,"BioSample"], layout = panicum_hallii[,"layout"], genome = "panicum_hallii", ploidy = 2, taxonKeys = "2705116"))
```
...
# add reads to growing list
# species is tetraploid, but the assembly has two separate subgenomes
allReads = rbind(allReads, data.frame(run = panicum_virgatum[,"Run"], replicate = panicum_virgatum[,"BioSample"], layout = panicum_virgatum[,"layout"], genome = "panicum_virgatum", ploidy = 2, taxonKeys = "2705162"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = papaver_somniferum[,"Run"], replicate = papaver_somniferum[,"BioSample"], layout = "SE", genome = "papaver_somniferum", ploidy = 2, taxonKeys = "2888443"))
```
...
any(duplicated(phaseolus_vulgaris$LibraryName))
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = phaseolus_vulgaris[,"Run"], replicate = phaseolus_vulgaris[,"BioSample"], layout = phaseolus_vulgaris[,"layout"], genome = "phaseolus_vulgaris", ploidy = 2, taxonKeys = "5350488"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = phoenix_dactylifera[,"Run"], replicate = phoenix_dactylifera[,"BioSample"], layout = "PE", genome = "phoenix_dactylifera", ploidy = 2, taxonKeys = "5293186"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = pisum_sativum[,"Run"], replicate = pisum_sativum[,"BioSample"], layout = pisum_sativum[,"layout"], genome = "pisum_sativum", ploidy = 2, taxonKeys = "5356701"))
```
...
# add reads to growing list
# not sure about the ploidy here
allReads = rbind(allReads, data.frame(run = populus_deltoides[,"Run"], replicate = populus_deltoides[,"BioSample"], layout = "PE", genome = "populus_deltoides", ploidy = 4, taxonKeys = "3040232"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = populus_trichocarpa[,"Run"], replicate = populus_trichocarpa[,"BioSample"], layout = populus_trichocarpa[,"layout"], genome = "populus_trichocarpa", ploidy = 2, taxonKeys = "8089775"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = prunus_armeniaca[,"Run"], replicate = prunus_armeniaca[,"BioSample"], layout = "PE", genome = "prunus_armeniaca", ploidy = 2, taxonKeys = "3021284"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = prunus_avium[,"Run"], replicate = prunus_avium[,"BioSample"], layout = "PE", genome = "prunus_avium", ploidy = 2, taxonKeys = "3020791"))
```
...
# add layout
#prunus_persica$layout = "PE"
prunus_persica$layout[(prunus_persica$spots_with_mates > 0)] = "PE"
prunus_persica$layout[(prunus_persica$spots_with_mates == 0)] = "SE"

# add reads to growing list
allReads = rbind(allReads, data.frame(run = prunus_persica[,"Run"], replicate = prunus_persica[,"BioSample"], layout = prunus_persica[,"layout"], genome = "prunus_persica", ploidy = 2, taxonKeys = "8324181"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = quercus_lobata[,"Run"], replicate = quercus_lobata[,"BioSample"], layout = quercus_lobata[,"layout"], genome = "quercus_lobata", ploidy = 2, taxonKeys = "2878373"))
```
...
# randomly subset to meet bp threshold
quercus_robur = random_subset(quercus_robur, maxbp = maxbp)

# add reads to growing list
allReads = rbind(allReads, data.frame(run = quercus_robur[,"Run"], replicate = quercus_robur[,"BioSample"], layout = "PE", genome = "quercus_robur", ploidy = 2, taxonKeys = "2880130"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = raphanus_sativus[,"Run"], replicate = raphanus_sativus[,"BioSample"], layout = "PE", genome = "raphanus_sativus", ploidy = 2, taxonKeys = "3040983"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = rhododendron_griersonianum[,"Run"], replicate = rhododendron_griersonianum[,"BioSample"], layout = "PE", genome = "rhododendron_griersonianum", ploidy = 2, taxonKeys = "4174687"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = salix_dunnii[,"Run"], replicate = salix_dunnii[,"BioSample"], layout = "PE", genome = "salix_dunnii", ploidy = 2, taxonKeys = "5582578"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = secale_cereale[,"Run"], replicate = secale_cereale[,"BioSample"], layout = "PE", genome = "secale_cereale", ploidy = 2, taxonKeys = "4121049"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = sesamum_indicum[,"Run"], replicate = sesamum_indicum[,"BioSample"], layout = "PE", genome = "sesamum_indicum", ploidy = 2, taxonKeys = "3172620"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = setaria_italica[,"Run"], replicate = setaria_italica[,"BioSample"], layout = setaria_italica[,"layout"], genome = "setaria_italica", ploidy = 2, taxonKeys = "7626470 5289666"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = setaria_viridis[,"Run"], replicate = setaria_viridis[,"BioSample"], layout = setaria_viridis[,"layout"], genome = "setaria_viridis", ploidy = 2, taxonKeys = "5289684"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = solanum_chilense[,"Run"], replicate = solanum_chilense[,"BioSample"], layout = "PE", genome = "solanum_chilense", ploidy = 2, taxonKeys = "5289684"))
```
...
solanum_lycopersicum$layout = "PE"
solanum_lycopersicum$layout[(solanum_lycopersicum$Run == "SRR1572576")] = "SE"
solanum_lycopersicum$layout[(solanum_lycopersicum$Run == "SRR1572544")] = "SE"
solanum_lycopersicum$layout[(solanum_lycopersicum$Run == "SRR1572368")] = "SE"
solanum_lycopersicum$layout[(solanum_lycopersicum$Run == "SRR1572267")] = "SE"
solanum_lycopersicum$layout[(solanum_lycopersicum$Run == "SRR1572268")] = "SE"
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = solanum_lycopersicum[,"Run"], replicate = solanum_lycopersicum[,"BioSample"], layout = solanum_lycopersicum[,"layout"], genome = "solanum_lycopersicum", ploidy = 2, taxonKeys = "2930397"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = solanum_melongena[,"Run"], replicate = solanum_melongena[,"BioSample"], layout = "PE", genome = "solanum_melongena", ploidy = 2, taxonKeys = "2930642 8402242"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = solanum_pimpinellifolium[,"Run"], replicate = solanum_pimpinellifolium[,"BioSample"], layout = "PE", genome = "solanum_pimpinellifolium", ploidy = 2, taxonKeys = "2931738"))
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = solanum_stenotomum[,"Run"], replicate = solanum_stenotomum[,"BioSample"], layout = "PE", genome = "solanum_stenotomum", ploidy = 2, taxonKeys = "3799678 9673874 4937418"))
```
...
# random subsample
sorghum_bicolor = random_subset(sorghum_bicolor, maxbp = maxbp)

# add reads to growing list
allReads = rbind(allReads, data.frame(run = sorghum_bicolor[,"Run"], replicate = sorghum_bicolor[,"BioSample"], layout = sorghum_bicolor[,"layout"], genome = "sorghum_bicolor", ploidy = 2, taxonKeys = "4108497"))
```
...
# remove pacbio data
spinacia_oleracea = spinacia_oleracea[(spinacia_oleracea$Platform == "ILLUMINA"),]
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = spinacia_oleracea[,"Run"], replicate = spinacia_oleracea[,"BioSample"], layout = spinacia_oleracea[,"layout"], genome = "spinacia_oleracea", ploidy = 2, taxonKeys = "11742811"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = spirodela_polyrhiza[,"Run"], replicate = spirodela_polyrhiza[,"BioSample"], layout = spirodela_polyrhiza[,"layout"], genome = "spirodela_polyrhiza", ploidy = 2, taxonKeys = "4097481"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = striga_hermonthica[,"Run"], replicate = striga_hermonthica[,"BioSample"], layout = "PE", genome = "striga_hermonthica", ploidy = 2, taxonKeys = "3171159"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = tetracentron_sinense[,"Run"], replicate = tetracentron_sinense[,"BioSample"], layout = "PE", genome = "tetracentron_sinense", ploidy = 2, taxonKeys = "5529117"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = thinopyrum_intermedium[,"Run"], replicate = thinopyrum_intermedium[,"BioSample"], layout = "SE", genome = "thinopyrum_intermedium", ploidy = 6, taxonKeys = "8395719"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = thlaspi_arvense[,"Run"], replicate = thlaspi_arvense[,"BioSample"], layout = "PE", genome = "thlaspi_arvense", ploidy = 2, taxonKeys = "3052680"))
```

## Trifolium pratense, DEPRECATED: study targets specific genes, 

other studies are pool-seq and I don't want to bother with that 

```{r eval=FALSE, include=FALSE}
trifolium_pratense = fread("../workflow/data/SRArunInfo/SraRunInfo_trifolium_pratense_PRJNA765476.csv.gz", data.table = F)

# add reads to growing list
allReads = rbind(allReads, data.frame(run = trifolium_pratense[,"Run"], replicate = trifolium_pratense[,"BioSample"], layout = "SE", genome = "trifolium_pratense", ploidy = 2, taxonKeys = "5358812"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = triticum_aestivum[,"Run"], replicate = triticum_aestivum[,"BioSample"], layout = triticum_aestivum[,"layout"], genome = "triticum_aestivum", ploidy = 6, taxonKeys = "2706411"))
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = triticum_dicoccoides[,"Run"], replicate = triticum_dicoccoides[,"BioSample"], layout = "SE", genome = "triticum_dicoccoides", ploidy = 4, taxonKeys = "4932502 7387585"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = triticum_turgidum[,"Run"], replicate = triticum_turgidum[,"BioSample"], layout = triticum_turgidum[,"layout"], genome = "triticum_turgidum", ploidy = 4, taxonKeys = "2706415"))
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = vanilla_planifolia[,"Run"], replicate = vanilla_planifolia[,"BioSample"], layout = "SE", genome = "vanilla_planifolia", ploidy = 2, taxonKeys = "2803330"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = vigna_radiata[,"Run"], replicate = vigna_radiata[,"BioSample"], layout = "SE", genome = "vigna_radiata", ploidy = 2, taxonKeys = "2982385"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = vigna_umbellata[,"Run"], replicate = vigna_umbellata[,"BioSample"], layout = "PE", genome = "vigna_umbellata", ploidy = 2, taxonKeys = "2982748"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = vigna_unguiculata[,"Run"], replicate = vigna_unguiculata[,"BioSample"], layout = "PE", genome = "vigna_unguiculata", ploidy = 2, taxonKeys = "2982409"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = vitis_vinifera[,"Run"], replicate = vitis_vinifera[,"BioSample"], layout = vitis_vinifera[,"layout"], genome = "vitis_vinifera", ploidy = 2, taxonKeys = "5832702"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = xanthoceras_sorbifolium[,"Run"], replicate = xanthoceras_sorbifolium[,"BioSample"], layout = "PE", genome = "xanthoceras_sorbifolium", ploidy = 2, taxonKeys = "7221300"))
```
...
# random subset
zea_mays = random_subset(zea_mays, maxbp = maxbp)

# add reads to growing list
allReads = rbind(allReads, data.frame(run = zea_mays[,"Run"], replicate = zea_mays[,"BioSample"], layout = zea_mays[,"layout"], genome = "zea_mays", ploidy = 2, taxonKeys = "5678457"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = zizania_palustris[,"Run"], replicate = zizania_palustris[,"BioSample"], layout = "SE", genome = "zizania_palustris", ploidy = 2, taxonKeys = "2703230"))
```
...
# add reads to growing list
allReads = rbind(allReads, data.frame(run = ziziphus_jujuba[,"Run"], replicate = ziziphus_jujuba[,"BioSample"], layout = "PE", genome = "ziziphus_jujuba", ploidy = 2, taxonKeys = "4928539"))
```
...
# paste(ls(), collapse = ", ")
runsAllMetaData = list(actinidia_chinensis, aegilops_tauschii, amaranthus_hypochondriacus, ananas_comosus, arabidopsis_halleri, arabidopsis_lyrata, arabidopsis_suecica, arabidopsis_thaliana, arabis_alpina, arabis_nemorensis, arachis_duranensis, arachis_hypogaea, arachis_ipaensis, benincasa_hispida, beta_vulgaris, boechera_stricta, brachypodium_distachyon, brassica_juncea, brassica_napus, brassica_nigra, brassica_oleracea, brassica_rapa, buddleja_alternifolia, cajanus_cajan, camelina_sativa, camellia_sinensis, cannabis_sativa, capsella_grandiflora, capsella_rubella, capsicum_annuum, castanea_mollissima, chenopodium_quinoa, cicer_arietinum, citrullus_lanatus, coffea_arabica, coffea_canephora, corylus_americana, corylus_avellana, cucumis_melo, cucumis_sativus, cucurbita_argyrosperma, cucurbita_pepo, digitaria_exilis, dioscorea_alata, dioscorea_rotundata, durio_zibethinus, elaeis_guineensis, eleusine_coracana, ficus_carica, glycine_max, glycine_soja, gossypium_arboreum, gossypium_barbadense, gossypium_hirsutum, helianthus_annuus, heliosperma_pusillum, hordeum_vulgare, juglans_regia, lactuca_sativa, lens_culinaris, lens_ervoides, linum_usitatissimum, lotus_japonicus, lupinus_angustifolius, macadamia_integrifolia, malus_domestica, malus_sylvestris, mangifera_indica, manihot_esculenta, medicago_truncatula, mimulus_guttatus, miscanthus_sinensis, momordica_charantia, musa_acuminata, nelumbo_nucifera, nicotiana_tabacum, olea_europaea, oryza_barthii, oryza_brachyantha, oryza_glaberrima, oryza_longistaminata, oryza_meridionalis, oryza_punctata, oryza_rufipogon, oryza_sativa, panicum_hallii, panicum_virgatum, papaver_somniferum, phaseolus_vulgaris, phoenix_dactylifera, pisum_sativum, populus_deltoides, populus_trichocarpa, prunus_armeniaca, prunus_avium, prunus_persica, quercus_lobata, quercus_robur, raphanus_sativus, rhododendron_griersonianum, salix_dunnii, secale_cereale, sesamum_indicum, setaria_italica, setaria_viridis, solanum_chilense, solanum_lycopersicum, solanum_melongena, solanum_pimpinellifolium, solanum_stenotomum, sorghum_bicolor, spinacia_oleracea, spirodela_polyrhiza, striga_hermonthica, tetracentron_sinense, thinopyrum_intermedium, thlaspi_arvense, vanilla_planifolia, vigna_radiata, vigna_umbellata, vigna_unguiculata, vitis_vinifera, xanthoceras_sorbifolium, zea_mays, zizania_palustris, ziziphus_jujuba)
```


# Get sequencing strategy and instrument for each sepcies
```{r}
get_seq_variables = function(x){
  species = unique(x$ScientificName)[1]
  strat = paste(unique(x$LibraryStrategy), collapse = ";")
  selec = paste(unique(x$LibrarySelection), collapse = ";")
  plat = paste(unique(x$Platform), collapse = ";")
  model = paste(unique(x$Model), collapse = ";")
  most_model = names(sort(table(x$Model), decreasing = T))[1]
  
  return(c(species, strat, selec, plat, model, most_model))
}

seq_meta = lapply(runsAllMetaData, FUN = get_seq_variables)

seq_meta = as.data.frame(do.call("rbind", seq_meta))

# remove subsp. and variety from species names
seq_meta$V1 = gsub(" subsp..*", "", seq_meta$V1)
seq_meta$V1 = gsub(" var..*", "", seq_meta$V1)

# Standardize sequencing method labels
# combine different methods of reduced representation sequencing
sort(table(seq_meta$V2))
seq_meta$method = seq_meta$V2
seq_meta$method[(seq_meta$method == "GBS")] = "RRS"
seq_meta$method[(seq_meta$method == "RAD-Seq")] = "RRS"
seq_meta$method[(seq_meta$method == "WXS" & seq_meta$V1 == "Zizania palustris")] = "RRS" # Zizania publication specifies restriction enzymes
seq_meta$method[(seq_meta$method == "OTHER" & seq_meta$V3 == "Restriction Digest")] = "RRS"
seq_meta$method[(seq_meta$method == "OTHER" & seq_meta$V3 == "Reduced Representation")] = "RRS"
seq_meta$method[(seq_meta$V1 == "Miscanthus sinensis")] = "RRS"
seq_meta$method[(seq_meta$V1 == "Lens culinaris")] = "RRS"
seq_meta$method[(seq_meta$V1 == "Helianthus annuus")] = "RRS"
seq_meta$method[(seq_meta$V1 == "Papaver somniferum")] = "RRS"

seq_meta$method[(seq_meta$method == "WGA")] = "WGS"
seq_meta$method[(seq_meta$method == "WGS;WGA")] = "WGS"
seq_meta$method[(seq_meta$V1 == "Quercus robur")] = "WGS"

# annotate which WGS studies include PCR
seq_meta$method[(seq_meta$V1 == "Oryza rufipogon")] = "WGS+PCR"
seq_meta$method[(grepl("PCR",seq_meta$V3) & seq_meta$method == "WGS")] = "WGS+PCR"

sort(table(seq_meta$method))

# combine platform labels
sort(table(seq_meta$V4))
seq_meta$platform = seq_meta$V4
seq_meta$platform[(seq_meta$platform == "BGISEQ")] = "OTHER"
seq_meta$platform[(seq_meta$platform == "ION_TORRENT")] = "OTHER"

# collapse model labels
sort(table(seq_meta$V6))
seq_meta$model = seq_meta$V6

seq_meta$model[grepl("HiSeq", seq_meta$model)] = "Illumina HiSeq"
seq_meta$model[(seq_meta$V1 == "Prunus persica")] = "Illumina HiSeq" # according to associated publication

seq_meta$model[(seq_meta$model %in% c("Illumina Genome Analyzer IIx", "NextSeq 500", "NextSeq 550", "Illumina Genome Analyzer"))] = "Other Illumina"

seq_meta$model[(seq_meta$model %in% c("DNBSEQ-T7", "Ion Torrent Proton", "MGISEQ-2000RS", "BGISEQ-500"))] = "Other Non-Illumina"

sort(table(seq_meta$model))

table(seq_meta$method, seq_meta$model)

# save labels
names(seq_meta)[1] = "species"
seq_meta$species[(seq_meta$species == "Erythranthe guttata")] = "Mimulus guttatus"

#write.table(seq_meta[,c("species", "method", "platform", "model")], "../results/2023-03-26/seq_methods_2023-10-20.txt", sep = "\t", quote = F, row.names = F)
```

# Loop over replicates, calculate coverage

If there are both SE and PE reads for a single sample, just use the PE reads
...
# filter out runs from metadata that didn't make it to final frame
#runsAllMetaData = runsAllMetaData[(runsAllMetaData$Run %in% allReadsFinal$run)]

# count total number of basepairs
...
# count number of bp sequenced per species
coverdf = NULL
for(x in runsAllMetaData){
  speciesdf = data.frame(
    totalbp = sum(as.numeric(x[,"bases"])), # total number of bp sequenced across all samples
    cvbp = sd(as.numeric(x[,"bases"]))/mean(as.numeric(x[,"bases"])), # coefficient of variation in bp b/w samples
    species = unique(x[,"ScientificName"])[1] # species name
  )
  coverdf = rbind(coverdf, speciesdf)
}

# change species names
coverdf$species = gsub(" var\\..*", "", coverdf$species)
coverdf$species = gsub(" subsp\\..*", "", coverdf$species)
coverdf$species[(coverdf$species == "Erythranthe guttata")] = "Mimulus guttatus"

write.table(coverdf, "../results/2023-03-26/bp_sequenced_2024-04-03.txt", sep = "\t", quote = F, row.names = F)
```

# DEPRECATED split data into batches
```{r eval=FALSE, include=FALSE}
sort(table(allReads$genome))

# batch 1: 40 species with the fewest samples
sum(sort(table(allReads$genome))[1:40])

sum(sort(table(allReads$genome))[41:55])
```
...
write.table(allReadsFinal, file = "../config/samples.tsv", sep = "\t", quote = F, row.names = F)

#write.table(allReads, file = "../config/samples.tsv", sep = "\t", quote = F, row.names = F)
