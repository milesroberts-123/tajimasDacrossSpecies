...
```{r}
# List input files
pi = list.files("../results/2023-03-14/heterozygosity", full.names = T)
bcd = list.files("../results/2023-03-14/bray-curtis-dissimilarities", full.names = T)
jac = list.files("../results/2023-03-14/jaccard-dissimilarities", full.names = T)

# get species IDs
species = gsub("_pairwise-pi.txt", "", gsub("../results/2023-03-14/heterozygosity/", "", pi))
...
alldiv = Map(cbind, pi, bcdMean, jacMean)

# rbind all species data into one dataframe
alldiv = lapply(alldiv, setNames, c("h", "n", "pi", "bcd", "jac"))
...
```{r}
phy = read.tree("speciesTree.nwk")
```

## Combine tree and data to get comparative object
```{r}
compdata = comparative.data(phy, alldiv, names.col = "species", vcv = TRUE)
```
...
```{r}
# How does SNP diversity correlate with k-mer diversity?
