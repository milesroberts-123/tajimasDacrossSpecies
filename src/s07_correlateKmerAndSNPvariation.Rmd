...
alldiv$speciesName = str_to_sentence(gsub("_", " ", alldiv$species))
```
...
```{r}
compdata = comparative.data(phy, alldiv, names.col = "speciesName", vcv = T)
```
...
pgls(log(pi) ~ bcd + nvariant, compdata)
pgls(log(pi) ~ jac + nvariant, compdata)
